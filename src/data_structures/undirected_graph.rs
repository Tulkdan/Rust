...
impl UndirectedGraph {
    ...
    pub fn add_edge(&mut self, origin: usize, destination: usize) -> Result<(), Errors> {
        ...
        let dest_value = self.nodes[destination].value;
        self.nodes[origin].pointers.push(dest_value);

        let ori_value = self.nodes[origin].value;
        self.nodes[destination].pointers.push(ori_value);
        Ok(())
    }
}
...
mod tests {
    ...
    fn should_add_edge_to_both_nodes() {
        ...
        assert_eq!(graph.nodes[1].pointers.len(), 1);
        assert_eq!(graph.nodes[1].pointers[0], 0);
    }
